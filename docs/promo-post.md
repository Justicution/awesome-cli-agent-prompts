...
**V2EX：**
```
折腾 OpenClaw 有感，把我的 Claude Code 配置重新捋了一遍，顺手开了个仓库
```

**LinuxDo：**
```
OpenClaw 用完感觉一般，但它让我想明白了一件事：我一直在用错 Claude Code
```
...
搞明白之后呢？说实话，能力上比不上 Claude Code、Gemini CLI 这些工具，差距还不小。但它有一点真的打动了我——对 system 层的设计方式。它把 Agent 的运行结构当作工程问题来处理，而不是一堆 prompt 技巧的堆砌。这个思路本身，比它的功能更值得借鉴。

结合我之前用 CLI Agent 的一些积累，重新把自己的配置整理了一遍，也就是下面要分享的东西。不是最佳实践，就是我目前觉得好用的设计，拿出来抛砖引玉。

---

你用 Claude Code 的时候，有没有遇到过这个：

每次开新会话，都要重新介绍一遍——"你叫什么，帮我做什么，风格是什么"，然后它花了一两个小时把项目摸透，session 一关，下次打开又是白纸。重新 paste，重新学，重新解释，重新忘。

我被这个循环折磨了挺久，后来意识到问题不是 Claude 不够聪明，是我一直把它当聊天窗口用，而不是当一个持续运行的系统。

聊天窗口是无状态的，每次对话都是第一次见面。
但一个真正好用的 Agent，应该每次启动都知道自己是谁、在帮谁、上次干到哪了。

做到这一点不需要什么黑科技，就是几个文件：
...
TOOLS.md      → 本地工具、环境配置
AGENTS.md     → 工作区规则、记忆体系、每次启动的仪式
HEARTBEAT.md  → 周期性心跳任务，可以让它主动做后台工作
BOOTSTRAP.md  → 第一次启动读完就删，出生证明
```

每次会话启动，Agent 读这几个文件，它就回家了。

记忆也是分层的，不是把所有东西塞进一个文件：
...
还有一条设计我觉得很关键，写在 AGENTS.md 里的：

> 不要先问许可。直接做。

大多数 Agent 配置卡在这里——说一句话，它问三个确认。这条规则配合安全边界（读文件直接做，发邮件/公开发布先问）用，主动但不乱来。
...
这套配置我用了几个月，兼容 Claude Code、Gemini CLI、OpenClaw、Codex CLI，原理都一样，文件落地方式略有差异。

我把它开源了，作为第一个提交，同时开了一个 GitHub 仓库想做成共创项目——
...
想法是：每个人提交自己真实在跑的 Agent 运行结构，互相看、互相投票，慢慢积累这个领域里真正有参考价值的东西。不是收 prompt，是收设计思路。

提交不需要"完整"，哪怕只有 SOUL.md + 一个 README 说清楚你的设计思路，就算有效。

现在仓库刚起步，就我一个提交，希望有人一起来填。

提交方式在这里：https://github.com/hengfengliya/awesome-cli-agent-prompts/blob/main/CONTRIBUTING.md

完整的 AGENTS.md 原文附在帖末，感兴趣可以直接抄。
